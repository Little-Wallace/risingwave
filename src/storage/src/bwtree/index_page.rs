...
impl IndexPage {
    ...
    pub fn get_largest_key(&self) -> Bytes {
    ...
    pub fn get_sub_tree(&self) -> Vec<(Bytes, PageId)> {
        let mut ret = Vec::with_capacity(self.sub_tree.len());
        for (k, v) in self.sub_tree.iter() {
            ret.push((k.clone(), *v));
        }
        ret
    }
